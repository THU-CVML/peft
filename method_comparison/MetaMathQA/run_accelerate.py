...
accelerate launch --config_file ./accelerate.yaml \
    --num_processes "$cuda_device_count" run_accelerate.py experiments/yuequ-ark/rank32-tanh
"""
...
def train(
    *,
    ...
    # Remove response column from training dataset
    ds_train_processed = ds_train.remove_columns(["response"])
    ...
    model, optimizer, lr_scheduler, iterator_train = accelerator.prepare(model, optimizer, lr_scheduler, iterator_train)

    try:
        model_inspect_informations = collect_model_inspect_informations(model, "model_inspect_information_init")
        logging_step = 0
        ...
        for step, batch in zip(pbar, iterator_train):
            ...
            swanlab.log({"batch train loss": loss.item()}, step=logging_step)
            logging_step += 1
